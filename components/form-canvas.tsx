...
interface FormCanvasProps {
  ...
  onAddElement: (type: FormElementType, rowId?: string, colIndex?: number, columnWidth?: ColumnCount) => void
  onAddRow: () => void
  onRemoveRow: (rowId: string) => void
  onMoveElement: (elementId: string, sourceRowId: string, targetRowId: string, targetIndex: number) => void
...
export default function FormCanvas({
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (source.droppableId !== destination.droppableId || source.index !== destination.index) {
      onMoveElement(draggableId, source.droppableId, destination.droppableId, destination.index)
    }
  }

  const selectedElement = formRows.flatMap((row) => row.elements).find((element) => element.id === selectedElementId)
  ...
  const addEmptyColumnsRow = (colCount: ColumnCount) => {
    ...
    const placeholderElements: FormElement[] = Array.from({ length: colCount }).map(
      (_, index) =>
        ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center gap-2">
              <div className="flex border rounded-md overflow-hidden">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={cn("rounded-none h-8 px-3", columnCount === 3 ? "bg-primary/10 text-primary" : "")}
                          ...
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={cn(
                                ...
                                activeRowId === row.id ? "border-primary bg-primary/5" : "border-gray-200",
                            ...
                            >
                              <CanvasRow
                                ...
                                setActiveRow={(active) => setActiveRowId(active ? row.id : null)}
      ...
      <div className="md:col-span-1">
        {selectedElement ? (
          <FormElementEditor element={selectedElement} onUpdateElement={onUpdateElement} />
        ) : (
          <Card>
            <CardHeader>
...
function CanvasRow({
  ...
  const [{ isOver }, drop] = useDrop({
    ...
    drop: (item: { type: FormElementType }) => {
      ...
      if (row.elements.some((el) => el.isPlaceholder)) {
        const placeholderIndex = row.elements.findIndex((el) => el.isPlaceholder)
  ...
  return (
    <div onMouseEnter={() => setActiveRow(true)} onMouseLeave={() => setActiveRow(false)} ref={drop}>
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center">
          <div {...dragHandleProps} className="cursor-grab p-1 mr-2 hover:bg-gray-100 rounded text-gray-500">
            <GripVertical size={16} />
          </div>
          <Grid size={16} className="mr-2 text-gray-500" />
          <span className="text-sm font-medium">Row {rowIndex + 1}</span>
          {columnCount > 1 && <span className="ml-2 text-xs text-gray-500">({columnCount} columns)</span>}
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
      ...
      <Droppable droppableId={row.id} direction="horizontal" type="element">
        {(provided, snapshot) => (
          <div
            ...
            } ${row.elements.length === 0 ? "border-2 border-dashed border-gray-200 rounded-md p-4" : ""}`}
          >
            {row.elements.length === 0 && !isOver ? (
              <div className="w-full text-center text-gray-500 text-sm">Drag elements here</div>
            ) : (
              row.elements.map((element, index) => {
                ...
                if (element.isPlaceholder) {
                  return (
                    ...
                    >
                      <p className="text-sm text-gray-500 text-center">Drop form elements here</p>
                    </div>
                  )
                }
                ...
                return (
                  <Draggable key={element.id} draggableId={element.id} index={index} type="element">
                    {(provided) => (
                      <div
                        ref={provided.innerRef}
                        {...provided.draggableProps}
                        className={`${widthClass} border rounded-md p-3 ${
                          selectedElementId === element.id ? "border-primary bg-primary/5" : "border-gray-200"
                        }`}
